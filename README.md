...
#### 7. 出现 `zsh: command not found: python` 怎么办？
...
后续遇到需要输入 `python` 的命令时，可以把 `python` 改成 `python3`。

例如：

    python3 -m venv .venv

    python3 pdf_desensitize.py ~/Desktop/原始PDF ~/Desktop/脱敏PDF
...
#### 7. What if I see `zsh: command not found: python`?
...
When a command uses `python`, you can use `python3` instead.

For example:

    python3 -m venv .venv

    python3 pdf_desensitize.py ~/Desktop/OriginalPDF ~/Desktop/RedactedPDF
